...
function collideSquare(myX, myY, xblock, yblock, possibleMoves) {
  if (myX + 1 == xblock
         && myY == yblock) possibleMoves.right = false
      if (myX - 1 == xblock
         && myY == yblock) possibleMoves.left = false
      if (myX == xblock
         && myY + 1 == yblock) possibleMoves.up = false
      if (myX == xblock
         && myY - 1 == yblock) possibleMoves.down = false
}

function collideTest(myHead, mybody, possibleMoves) {
    for (let index = 0; index < mybody.length; index++) {
      const xblock = mybody[index].x
      const yblock = mybody[index].y

      collideSquare(myHead.x, myHead.y, xblock, yblock, possibleMoves)
    }  
}
...
function getPossibleMoves(gameState) {
  ...
  // but the algorithm or lookahead needs it separately!!
    ...
    collideTest(myHead, mybody, possibleMoves)
    ...
    for (let index = 0; index < snakes.length; index++) {
      if (snakes[index].id != gameState.you.id)
        collideTest(myHead, snakes[index].body, possibleMoves)
    }
...
function move(gameState) {
  ...
  const longest = Math.max(...snakeLengths)
      ...
      longerSnakeHeads = otherSnakes.filter(s => s.length >= gameState.you.length).
        map(s => s.head)
  ...
  let possibleMovesAvoidingHeads = avoidLongHeads(myHead, longerSnakeHeads)
    ...
    let isAttacking = snakes.length > 1 &&
      gameState.you.length > longest + 1
